...
- macOS local presets: dev, release, ASan/UBSan, TSan, and profile all pass 73/73 tests
- Ubuntu 24.04 Docker release run passes 98/98 tests, including Linux-only epoll and CLI smoke tests
...
writes, byte budgets, queue saturation, worker pool shutdown, worker handler exceptions, stale response discard, response
